...
const getDatas = async (req, res) => {
  ...
  const findDatas = await commissionerCandidateModel.findAll({ where });
...
const createData = async (req, res) => {
  ...
  const photo_url = `/images/commissioner_candidates/${photo_name}`;
  ...
  file.mv(
    `./public/images/commissioner_candidates/${photo_name}`,
    async (err) => {
      if (err) return res.status(500).json({ message: err.message });
      try {
        const commissioner_candidate = await commissionerCandidateModel.create({
        ...
        await createLogHandler({
          user_id: req.user.id,
          activity: "commissioner_candidate-create",
...
const updateData = async (req, res) => {
  ...
  if (!req.files || !req.files.file || req.files.file.length === 0) {
    const commissioner_candidate = await findData.update({
      voting_period_id: findVotingPeriod.id,
      name,
      vision,
      mission,
    ...
    await createLogHandler({
      user_id: req.user.id,
      activity: "commissioner_candidate-update",
      description: `${req.user.name} has updated commissioner candidate ${name}`,
    });

    return res.status(201).json({
      status: 201,
      success: true,
      datas: commissioner_candidate,
    });
  }
  ...
  if (findData.photo_name !== null) {
    const filePath = `./public/images/commissioner_candidates/${findData.photo_name}`;
...
const deleteData = async (req, res) => {
  ...
  if (Boolean(permanent) === true) {
    ...
    await createLogHandler({
      user_id: req.user.id,
      activity: "commissioner_candidate-delete",
      description: `${req.user.name} has deleted commissioner candidate ${findData.name}`,
    });
