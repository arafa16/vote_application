...
const StatusDataEditPage = () => {
  ...
  useEffect(() => {
    if (dataStatus !== null && isSuccessStatus && !isLoadingStatus) {
      setFormData({
        ...
        sequence: dataStatus?.data?.sequence,
        code: dataStatus?.data?.code,
      });
  ...
  const handleSubmit = () => {
    dispatch(UpdateStatusData({ uuid: id, formData }));
  };
...
export default StatusDataEditPage;
