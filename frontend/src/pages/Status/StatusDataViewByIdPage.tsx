...
const StatusDataViewByIdPage = () => {
  ...
  useEffect(() => {
    if (dataStatus !== null && isSuccessStatus && !isLoadingStatus) {
      setDataView(dataStatus?.data);
      dispatch(resetStatus());
    } else if (messageStatus !== "" && isErrorStatus && !isLoadingStatus) {
      dispatch(resetStatus());
    } else if (messageStatus !== "" && isSuccessStatus && !isLoadingStatus) {
      navigate("/status");
      dispatch(resetStatus());
    } else if (messageStatus !== "" && isErrorStatus && !isLoadingStatus) {
      dispatch(resetStatus());
  ...
  const handleEdit = () => {
    navigate(`/status/data/${id}/edit`);
  };

  const handleDelete = () => {
    dispatch(DeleteStatusData({ uuid: id }));
  };
...
export default StatusDataViewByIdPage;
