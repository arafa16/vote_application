...
const AuditLogDataEditPage = () => {
  ...
  useEffect(() => {
    if (dataAuditLog !== null && isSuccessAuditLog && !isLoadingAuditLog) {
      setFormData({
        ...formData,
        user_uuid: dataAuditLog?.data?.user?.uuid,
        activity: dataAuditLog?.data?.activity,
        description: dataAuditLog?.data?.description,
      });
      setAttributeData(dataAuditLog?.attributes);
  ...
  const handleSubmit = () => {
    dispatch(UpdateAuditLogData({ uuid: id, formData }));
  };

  useEffect(() => {
    dispatch(GetUpdateAuditLogAttributes(id));
  }, [dispatch, id]);
...
export default AuditLogDataEditPage;
