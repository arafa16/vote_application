...
const AuditLogDataTablePage = () => {
  ...
  useEffect(() => {
    if (dataAuditLog !== null && isSuccessAuditLog && !isLoadingAuditLog) {
      setDataTable(dataAuditLog?.data);
      setMetaTableData(dataAuditLog?.meta);
  ...
  useEffect(() => {
    if (search !== "") {
      const handler = setTimeout(() => {
        ...
        dispatch(GetAuditLogTable(searchParams));
      }, 500);
  ...
  const handleCreate = () => {
    navigate("/audit_log/create");
  };

  const handleClickData = (id: any) => {
    navigate(`/audit_log/data/${id}`);
  };
    ...
    <div>
      ...
      <div className="w-full">
        <div className="grid grid-cols-12 mb-2">
          <div
            className={`col-span-2 md:col-span-1 h-1 ${!isLoadingAuditLog ? "hidden" : ""}`}
          >
            <LoadingIcon
              icon="three-dots"
              color="#005266"
            ...
            <Table>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th className="whitespace-nowrap">Activity</Table.Th>
                  <Table.Th className="whitespace-nowrap">Description</Table.Th>
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody>
...
export default AuditLogDataTablePage;
