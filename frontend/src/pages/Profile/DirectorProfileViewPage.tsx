...
const DirectorProfileViewPage = () => {
  ...
  useEffect(() => {
    if (
      ...
      handleDirectorCandidate(dataVotingPeriod?.data[0]?.uuid);
  ...
  useEffect(() => {
    handleDirectorCandidate(votingPeriodSelected);
  ...
  return (
    <div>
      ...
      <div className={`w-full ${votingPeriodSelected === "" ? "hidden" : ""} `}>
        <div
          className={`w-full bg-primary rounded-md py-2 px-4 mb-4 ${votingPeriodSelected === "" ? "hidden" : ""}`}
        >
          <p className="text-[14px] text-white">KANDIDAT PENGURUS </p>
        </div>
        <div
          className={`w-full ${votingPeriodSelected === "" ? "hidden" : ""}`}
        >
          {directorDatas?.map((data, index) => (
            <div className="mb-4" key={index}>
              <CandidateView data={data} number={1} color="primary" />
            </div>
          ))}
        </div>
      </div>
...
export default DirectorProfileViewPage;
