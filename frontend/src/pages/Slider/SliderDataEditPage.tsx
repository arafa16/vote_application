...
const SliderDataEditPage = () => {
  ...
  useEffect(() => {
    if (dataSlider !== null && isSuccessSlider && !isLoadingSlider) {
      setFormData({
        ...formData,
        page_view: dataSlider?.data?.page_view,
        sequence: dataSlider?.data?.sequence,
      });
  ...
  const handleSubmit = () => {
    ...
    dispatch(UpdateSliderData({ uuid: id, formData: formDataSubmit }));
  };
...
export default SliderDataEditPage;
